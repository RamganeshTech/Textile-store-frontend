...
const Products = () => {
  ...
  const [rating, setRating] = useState<number>(3.5); // Example rating
  ...
  const renderStars = (rating:number) => {
      ...
      for (let i = 1; i <= 5; i++) {
          if (i <= rating) {
              stars.push(<StarIcon key={i} className={`${style.staricon}`} sx={{ fill: "gold" ,
                // fontSize: "clamp(16px, 2vw, 24px)" 
              }} />);
          } else if (i - 0.5 === rating) {
              stars.push(<StarIcon key={i} className={`${style.staricon}`} sx={{ fill: "gold", opacity: 0.5,
                // fontSize: "clamp(16px, 2vw, 24px)" 
               }} />);
          } else {
              stars.push(<StarBorderIcon key={i} className={`${style.staricon}`} sx={{ fill: "gray",
                // fontSize: "clamp(16px, 2vw, 24px)" 
               }} />);
          }
      }
  ...
  return (
    ...
    <div className={`${style.descriptioncontainer}`}>
        ...
        <p>M.R.P <span>₹</span><span>999</span></p>
